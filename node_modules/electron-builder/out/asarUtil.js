    ...
    compileUsingElectronCompile(files) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            yield (_bluebirdLst2 || _load_bluebirdLst2()).default.map(files, function (file) {
                if (file.indexOf("/node_modules/") !== -1 || file.indexOf("/bower_components/") !== -1 || !(file.indexOf("/", nextSlashIndex // ignore not root files 
                ) !== -1) || !metadata.get(file).isFile()) {
                    return null;
                }
            ...
            for (let i = 0, n = files.length; i < n; i++) {
                ...
                if (autoUnpackDirs.has(packageDir)) {
                    ...
                    if (fileParent !== packageDir && !autoUnpackDirs.has(fileParent)) {
                        ...
                        addValue(dirToCreate, _path.relative(_this3.src, packageDir), _path.relative(packageDir, fileParent));
                    }
                    continue;
                }
                ...
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`${_path.relative(_this3.src, packageDir)} is not packed into asar archive - contains executable code`);
