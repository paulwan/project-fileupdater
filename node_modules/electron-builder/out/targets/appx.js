...
class AppXTarget extends (_electronBuilderCore || _load_electronBuilderCore()).Target {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if ((yield packager.cscInfo.value) == null) {
            ...
            yield (_bluebirdLst2 || _load_bluebirdLst2()).default.all([(_bluebirdLst2 || _load_bluebirdLst2()).default.map(["44x44", "50x50", "150x150", "310x150"], function (size) {
                ...
                if (resourceList.indexOf(`${size}.png`) !== -1) {
                    return (0, (_fs || _load_fs()).copyFile)(_path.join(packager.buildResourcesDir, `${size}.png`), target);
                }
                return (0, (_fs || _load_fs()).copyFile)(_path.join(vendorPath, "appxAssets", `SampleAppx.${size}.png`), target);
            ...
            packager.dispatchArtifactCreated(destination, _this, arch, packager.expandArtifactNamePattern(_this.options, "appx", arch, "${name}-${version}-${arch}.${ext}"));
        })();
    }
            ...
            const manifest = (yield (0, (_fsExtraP || _load_fsExtraP()).readFile)(_path.join(templatePath, "appxmanifest.xml"), "utf8")).replace(/\$\{([a-zA-Z]+)\}/g, function (match, p1) {
                switch (p1) {
                    ...
                    case "publisherDisplayName":
                        const name = _this2.options.publisherDisplayName || appInfo.companyName;
                        if (name == null) {
